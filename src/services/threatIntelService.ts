...
// Enhanced fetch with timeout for crypto APIs - ALWAYS uses proxy for external URLs
async function fetchCryptoAPI(url: string, options: RequestInit = {}, timeout = 15000): Promise<Response> {
  const controller = new AbortController();
  const id = setTimeout(() => controller.abort(), timeout);
  
  const finalUrl = isExternalUrl(url) && !url.includes('allorigins') && !url.includes('corsproxy') 
    ? withProxy(url) 
    : url;
  ...
  try {
    ...
    // Try alternative proxy on failure
    if (isExternalUrl(url) && !url.includes('corsproxy')) {
      console.log('[CryptoAPI] Primary proxy failed, trying alternative...');
      try {
        const altUrl = `${ALT_CORS_PROXY}${encodeURIComponent(url)}`;
        ...
        console.error('[CryptoAPI] Alternative proxy also failed:', altError);
      }
    }
...
// Query Etherscan API (Free tier) for Ethereum - uses proxy
async function queryEtherscan(address: string): Promise<any> {
  try {
    console.log(`[Etherscan] Querying Ethereum: ${address}`);
    
    // Get balance - using fetchCryptoAPI with proxy
    const balanceResponse = await fetchCryptoAPI(`https://api.etherscan.io/api?module=account&action=balance&address=${address}&tag=latest`);
    ...
    // Get transaction count - using fetchCryptoAPI with proxy
    const txCountResponse = await fetchCryptoAPI(`https://api.etherscan.io/api?module=proxy&action=eth_getTransactionCount&address=${address}&tag=latest`);
