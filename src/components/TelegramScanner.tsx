...
export function TelegramScanner() {
  ...
  useEffect(() => {
    // Auto-scan default channels on mount
    if (scanResults.length === 0) {
      quickScanDefaults();
    }
  }, []);

  const quickScanDefaults = async () => {
    setLoading(true);
    try {
  ...
  const avgSensitivity = selectedResult 
    ? selectedResult.messages.reduce((sum, m) => sum + m.sensitivityScore, 0) / selectedResult.messages.length 
    : 0;
    ...
    <div className="space-y-6 p-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button onClick={quickScanDefaults} variant="outline" disabled={loading}>
